...
async function GetList() {
    ...
    try {
        ...
        const xmlData = await fs.promises.readFile(listFile, "utf8");
        const result = await xml.parseStringPromise(xmlData, { explicitArray: false });

        for (const manga of result.myanimelist.manga) {
            // console.log(manga.manga_title + "|" + manga.my_status);
            output.push({ title: manga.manga_title, status: manga.my_status });
        }
